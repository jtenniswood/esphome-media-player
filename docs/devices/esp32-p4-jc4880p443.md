...
For a 90-degree landscape mount, use the rotated layout package:
...
This still uses ESPHome 2026.4's native LVGL rotation (`lvgl.rotation`). The package only swaps the layout values to an 800×480 canvas, with artwork on the left and the accent-coloured track panel on the right.
