...
4. **Check the image format and size** — artwork is decoded on the device. Baseline JPEG, progressive JPEG, and PNG are handled by the native firmware decoder. HEIC/HEIF is detected but is not displayed by the current native firmware decoder. Very large images can also fail on the ESP32's limited memory. When reporting this, include the media provider and the device logs.
